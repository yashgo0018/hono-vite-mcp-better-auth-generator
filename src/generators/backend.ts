...
export function generateBackend(
  ...
  if (config.includeAuth) {
    ...
    if (config.includeDatabase) {
      deps["@better-auth/drizzle-adapter"] =
        versions.get("@better-auth/drizzle-adapter") || "^1.3.12";
    }
  }
  ...
  if (config.includeMcp) {
    ...
    if (config.includeMcpOAuth) {
      deps["@better-auth/oauth-provider"] =
        versions.get("@better-auth/oauth-provider") || "^1.4.18";
    }
  }
